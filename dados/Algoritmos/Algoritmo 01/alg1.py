...
def parentSelection(populacao):
	...
	return tournament(populacao, 2, 5)
...
def mutation(filho):
	...
	mutante = scramble(filho) if probability(PRO_MUTACAO) else filho
...
def avaliation(num_testes):
	...
	for i in range(num_testes):
		...
		qtd_convergidos = []
		fitness_med = []
		ofile  = open('alg1teste'+str(i)+'.csv', "w", newline='')
		writer = csv.writer(ofile)
		writer.writerow(['config','fitness'])
		print("Teste " + str(i + 1))
		...
		ofile  = open('alg1info'+str(i)+'.csv', "w", newline='')
		writer = csv.writer(ofile)
